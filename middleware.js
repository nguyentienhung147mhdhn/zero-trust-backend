  ...
  try {
    ...
    if (currentHour < 8 || currentHour >= 17) {
      return res.status(403).json({ 
        message: "Cảnh báo (403 Forbidden): Hệ thống chỉ cho phép truy cập trong giờ hành chính (8h00 - 17h00)" 
      });
    }
    ...
    if (decoded.loginIp && currentIp !== decoded.loginIp) {
      return res.status(403).json({ 
        message: "Cảnh báo (403 Forbidden): Địa chỉ IP bị thay đổi bất thường. Vui lòng đăng nhập lại" 
      });
    }
